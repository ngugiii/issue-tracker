    ...
    <div className="px-24 flex w-full">
      <div className="w-full">
        ...
        {Object.keys(groupedIssues).map((key) => (
            ...
            >
              <span className="lowercase">{key} ISSUES</span>  <span>{groupedIssues[key]}</span>
            </div>
          ))}
        </div>
      ...
      <div className="border-2 ml-4 w-[50%] border-gray-300 p-4 rounded-lg">
        <h1 className="text-xl flex items-center mb-2 text-[orangered]">Latest Issues<span className=" font-normal text-sm text-black ml-1"> (issues registered today)</span></h1>
        {todayIssues.length>0 ? todayIssues.map((issue)=>(
